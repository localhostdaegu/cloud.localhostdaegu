...
- ✅ `OPN_ATMY_GRP_CD` 코드표(§11) 2026-09-16 실응답으로 확정 — 8구·군 주소가 기대 구·군과 일치 (일반음식점 total: 중구 9,628)
...
|---|---|---|---|
| 전통시장 현황 | **브이월드 데이터 API `LT_P_TRADSIJANG`** (data.go.kr 15122590이 이 API로 연결됨) — `VWORLD_API_KEY`로 호출, **활용신청 불요**. `api.vworld.kr/req/data?service=data&request=GetFeature&data=LT_P_TRADSIJANG&geomFilter=BOX(128.35,35.60,128.77,36.02)&size=1000` (attrFilter 단일검색 불가 → bbox 필터 후 주소 '대구' 2차 필터). 2026-09-16 실측 **107개**(bbox 113) — 속성 name·category·adr_road·adr_jibun·market·items·giftcard·toilet·park·opn_per·tel_num, Point 좌표. 점포수 없음 → 파일 15052836으로 보강 | — | 전통시장 좌표 → "시장 반경 500m" 파생변수 |
| 온누리상품권 가맹점 (소진공) | 파일 **3060079** 전국 온누리상품권 가맹점 (키 불요, 다운로드) | 3060079 | 전통시장 상권 활성도 프록시 |
...
- ✅ 2026-09-17 조회 방식 변경: 8구·군 결과가 전부 동일 63건(대구 구 전용 창업 정책 현재 0건)이고 호출마다 400·403·500 간헐 오류 → **`zipCd` 없이 전국 1회 조회**(`pageSize=500` 허용, 전국 창업 340건) 후 항목 `zipCd`에 대구 구·군 코드(**군위 27720 포함**)가 있는 것만 취함. 클라이언트 필터 결과가 서버 필터 63건과 일치. 전국엔 단일 구·군 전용 정책이 105건이라 대표 구 1회 조회는 누락 위험. HTTP 오류·타임아웃은 4xx 포함 지수 백오프 4회 재시도
...
| 구·군 | `LAWD_CD` (실거래가) | `OPN_ATMY_GRP_CD` (인허가) — **2026-09-16 8개 전부 실응답으로 확정** |
|---|---|---|
| 중구 | 27110 | 3410000 |
| 동구 | 27140 | 3420000 |
| 서구 | 27170 | 3430000 |
...
- [x] 인허가 대구 1건 실호출 → `OPN_ATMY_GRP_CD` 실값 확정 → §11 갱신 — 2026-09-16 활용신청 반영 후 8코드 확정. 거리두기 15098772·보조금24 15113968은 아직 미등록(P2)
- [x] R-ONE 대구 상권 목록 1회 호출 → 상권 수·빈티지 확인 — 2026-09-16 적재 1,572행, 최신 빈티지 상권 24개 (상권 단위 유지)

## 16. 검증 게이트 (수집 완료 판정 기준)

- [x] 인허가: 구·군별 건수 > 0, `CLSBIZ_YMD` 파싱률 확인, 좌표 변환 후 대구 bbox(§11) 내 비율 ≥ 95% — 2026-09-16 161,115건 / 폐업일 99.98% / bbox 100%
- [ ] 실거래가: 9개 `LAWD_CD` × 최근 12개월 응답 성공률
- [x] 전통시장: 대구 내 시장 수가 시 공식 통계(약 100여 개)와 대략 일치 — 2026-09-16 브이월드 LT_P_TRADSIJANG 107개 (적재는 미구현)
