...
- **한계·미결**: ① **담배소매인 ≠ 편의점** — 슈퍼·마트 등 담배 판매점이 함께 잡히고, 담배를 안 파는 편의점은 빠진다. 6,208은 편의점 점포 수가 아니라 지정 건수다. ② 어린이집 폐업률은 전 지역 0이라 위험도의 폐업 성분이 동률이다(학원·부동산과 같은 상태) — 스냅샷 2~4주 후 재평가. ③ 입소대기 1,418명은 중복 신청을 포함한 원천 건수이고 공란 시설은 합산에서 빠진다(전부 공란이면 "미공개"). ④ 어린이집의 나머지 두 카드 라벨("폐업률"·"점포 증감률")은 그대로 뒀다. ⑤ **미커밋**.
...
- **하지 않은 것·미결**: ① 리뷰 경로(`purpose=review`)의 등급 문구 "진입 주의"는 그대로 — 모델 평가 하네스(`direction_ok`)가 이 단어에 걸려 있다. ② 모바일 패널의 하단 고정 버튼은 패널을 스크롤해야 보인다. ③ 자동 시작 뒤에도 버튼으로 다시 만들 수 있다(의도된 동작이지만 LLM 호출이 또 나간다). ④ **미커밋·프론트 미배포** — 백엔드는 재시작해 운영에 반영됨. 운영 프론트는 이전 빌드라 추가 필드·라벨 변경과 섞여 돌고 있다(추가만 한 계약이라 동작은 정상, "점포 증감률" 카드 설명만 빠진다).
