...
### 미연결 데이터 붙이기 — 인구·동네 특성·임대료·충격 이벤트, OPEN-008·009 반영

- **OPEN-008 집계 반영**: 개업 후 30일 이내 종료 건을 개폐업 집계에서 제외(`apps/metric/domain/short_lived.py` `SHORT_LIVED_MAX_DAYS=30`, `StoreStatsGateway.yearly_stats` WHERE 절 — 개업일·폐업일이 없는 행은 그대로 둔다). `build_metrics` 재실행 10,112행. **실측(전 업종 합계)**: 2025년 개업 5,755 → 4,780 · 폐업 6,259 → 5,301, 2024년 폐업 5,905 → 5,256, 2023년 5,831 → 5,328. **폐업률 100% 초과 2건 → 0건**. 대신동 카페는 폐업 35건이 전부 30일 초과라 57.4% 그대로(점포 66 → 65). 재실행 전 테이블 덤프: `data/cache/region_industry_metric.before-open008.sql`(git 미추적).
- **OPEN-009 화면 반영**: "진입 고위험/주의/양호" → "상대 위험 높음/중간/낮음" + "대구 142개 동 가운데 2번째로 높아요. 동네끼리 비교한 상대 점수이며 실패 확률이 아닙니다". 성분 라벨도 "경쟁밀도 기여" → "점포 수 순위"(면적·인구로 나눈 밀도가 아니므로 — 이어받기 §0-2).
- **백엔드 읽기 슬라이스 3개**(11-파일 세트, 각 `/myself` 포함, 병렬 서브에이전트 작성 후 검토): `GET /populations/{region_code}/summary`(기준 2020-12 → 최신, 4개 연령대 M+F 합) · `GET /indicators?region_code=`(지표 키별 최신 기간) · `GET /rents/latest`(R-ONE 최신 분기, 천원/㎡ — 환산 없음). `main.py`에 3개 라우터 연결, `localhostdaegu-backend` 재시작, 공개 주소 `/rents/myself` 200 확인.
- **지도 패널**: 추이 차트에 업종별 영향 high 사건만 ▼로 표시(카페: 2020 대구 첫 확진·2021 거리두기 3단계·재강화 — 매년 있는 최저임금 medium은 제외). 주민등록 인구 블록(대신동 8,094 → 7,263명 −10.3%, 60세 이상만 +6.4%, "방문객·매출로 바꿔 읽지 마세요" 고지), 동네 특성 배지(전통시장 8·백년가게 5·나들가게 2·온누리 가맹점 1,820), 지하철 시간대별 승차(역이 있는 56개 동만).
- **사전상담**: "월세를 아직 모르겠다면" 도우미 — R-ONE 상권(대구 평균 + 개별 상권 × 소규모/중대형)과 평수를 고르면 ㎡당 임대료 × 면적을 만원 단위로 내려 월세 칸에 넣는다(대구 평균 소규모 10평 → 67만원). **`rent_price`에는 행정동·자치구 매핑이 없어(`district_code` 786행 전부 NULL) 선택한 동으로 자동 입력은 못 한다** — 사용자가 상권을 고르게 했다.
- **랜딩 칩**: 역매칭이 없는 "예산 5천이면 뭐 하지?" → 끝까지 이어지는 "수성구에서 헬스장, 예산 8천"(실제 파싱 A유형·27260·gym·8천만 확인).
- **검증**: 백엔드 pytest **558 passed / 1 skipped**(526 → +32), 프론트 vitest **212 passed**, `tsc` 0, 실백엔드 `tests/funnel.cjs` RESULT: PASS, 헤드리스 스크린샷 확인.
- **하지 않은 것·미결**: ① `/funding` 목록 화면 — 1,735건 전국 공고라 관련도 필터 없이는 소음(리포트 RAG가 이미 쓴다). ② 편의점·어린이집 지표 연결, 학원 수강료 — 별도 테이블→`store` 통합이 필요해 미착수. ③ 예산 역매칭 — 공정위 창업비용 데이터 없음. ④ mock API(`/api/mock`)에 새 3개 엔드포인트 없음 — mock 모드에서는 해당 블록이 조용히 빠진다. ⑤ **제안서 `docs/plan/plan.md` §58의 "2025년 개업 4,854 · 폐업 6,259"는 규칙 반영 전 7업종 원수치** — 화면 수치와 기준이 달라졌으니 제출 전 문구 확인 필요. ⑥ 이 절의 변경은 **미커밋**(백엔드 서비스는 작업 트리 코드로 이미 재시작됨).

### 내부 화면 다듬기 — 사전상담 2단·자금 그래프·지도 추이 차트·동 이름 표기

- **배경**: redoceanmap 비교 분석 결과 상권분석 전면 이식·전면 대화형 전환은 하지 않기로 했다(서울 추정매출·유동인구 의존, 9/18 자금 사전상담 전환과 어긋남). 대신 첫 화면에 비해 폼과 글만 있던 지도·사전상담·AI 분석 화면을 다듬었다. **백엔드 변경 없음, 신규 npm 의존성 없음**(차트는 SVG·CSS로 직접 그렸다 — recharts는 jsdom에서 렌더가 비어 테스트가 어렵다).
- **사전상담**: 한 열 약 4,600px → 넓은 화면에서 입력(좌)·결과(우, sticky) 2단. 주 지표 글자 확대 + 현재안을 볼 때 **최초안 대비 ▼▲ 금액**. 자금 구성 누적 막대(자기자본 · 희망대출(미확보) · 남는 부족액), 3시나리오 막대 + 손익분기 점선, **금리 +1%p/+2%p 표**(엔진 응답에 있던 `stress`가 처음 화면에 나온다 — 희망대출 0원이면 그리지 않는다). 상품 카드는 절차·서류를 `<details>`로 접고 3열.
- **지도 패널**: 2019~2026 점포 수 막대 + 폐업률 선(`RegionTrend`). 지도와 같은 쿼리 키(`["metrics", metric, industry, year]`)라 받은 연도는 재요청하지 않는다. **2026년은 집계 중**이라 막대는 흐리게, 폐업률 선에서는 뺐다. 패널이 페이지 높이를 밀어 범례가 화면 밖으로 나가던 것을 패널 내부 스크롤 + 하단 고정 CTA로 바꿨다. 10자리 행정동 코드 노출 제거(동 이름은 지도 경계 캐시 `["geojson"]` 재사용 — `useRegionNames`).
- **수치 신뢰 표기**: 폐업률·성장률 카드에 분모 설명 추가("전년 말 점포 수 대비 그해 폐업 건수 · 개업 30일 내 종료 포함"). 범례 최상위 구간을 "22.7% 이상"으로 열었다 — 달성군 1개 동(점포 51·폐업 70 → 122.8%)이 구간 끝값으로 읽히던 문제. 실측: 대신동 카페 점포 수 2019년 121개 → 2025년 66개, 폐업률 최고 2024년 64.6%.
- **AI 분석**: "지역 코드" 입력칸 → 행정동 이름 select, 업종도 select. 제출값은 그대로 코드·id라 API 계약 불변. 상담자료 메타의 지역 표기도 동 이름.
- **검증**: vitest **203 → 209 passed**(신규 6: 자금 막대·금리 표·전후 차이·추이 문장·동 이름 select), `tsc` 0, 실백엔드 `tests/funnel.cjs` **RESULT: PASS**(①~⑨), 헤드리스 스크린샷으로 3개 화면 육안 확인. 기존 단언 2건은 같은 금액이 막대 범례에도 나와 주 지표 칸을 짚도록 좁혔다.
- **미결**: ① 위험도 라벨 "진입 고위험/주의/양호"는 그대로 — OPEN-009(상대 순위 표기, 장민석) 몫. ② 폐업률 자체의 30일 내 종료 제외는 집계 코드 몫(OPEN-008, 류준) — 화면은 "포함"이라고 밝히기만 한다. ③ 적재만 되고 소비처 없는 데이터: `population_stat` 48,174 · `academy_course` 21,646 · `regional_indicator` 2,933(지하철 승차 2,632 + 시장·나들가게·백년가게·온누리 301) · `rent_price` 786 · `shock_event` 29(API는 있으나 호출처 없음) · 편의점 2,019·어린이집 986(지표 미연결). 전부 새 API가 필요해 본선용. ④ 랜딩 칩 "예산 5천이면 뭐 하지?"는 역매칭 없이 지도로만 간다. ⑤ `scripts/e2e-journey.sh`는 라벨만 맞췄다 — "AI 분석 →" 클릭 단계가 이미 옛 CTA라 스크립트 자체가 낡았다. ⑥ **미커밋·미배포** — Vercel 반영은 커밋·푸시 후.

### 첫 배포 — Vercel(프론트) + Cloudflare Tunnel(백엔드), 네임서버 이전

- **배포 전 점검**: 백엔드 **526 passed / 1 skipped**, 프론트 vitest 203 passed, `tsc` 0, `next build` exit 0, `alembic check` 드리프트 없음(head `e3f4a5b6c7d8`). 막힌 것은 코드가 아니라 인프라 5건(가비아 네임서버·터널 부재·Vercel 미연결·`CORS_ALLOW_ORIGINS` 없음·운영 `NEXT_PUBLIC_API_BASE` 없음).
- **계정 분리**: Cloudflare·Vercel 모두 프로젝트 전용 새 계정. 기존 remakeday 인증서·다른 Vercel 로그인을 덮지 않도록 인증서는 `~/.cloudflared/localhostdaegu-cert.pem`, Vercel 토큰은 `~/.config/vercel-localhostdaegu`(`--global-config`)에 둔다. **함정**: `cloudflared tunnel --origincert <경로> login`은 기본 `cert.pem`이 있으면 거부한다 — `HOME`을 임시 폴더로 돌려 로그인한 뒤 옮겼다.
- **DNS**: Cloudflare 온보딩의 자동 스캔은 **0건**을 찾았다 → `053` CNAME(`localhostdaegu.github.io`, DNS 전용)을 수동 추가한 뒤 가비아 네임서버를 `harley`·`tessa.ns.cloudflare.com`으로 교체. 가비아 저장 → 등록소 반영 약 10분, Cloudflare 활성화까지 추가 3분. 블로그 `053`은 전 구간 200 유지. 루트는 `A @ 76.76.21.21`(DNS 전용), `api`는 터널 명령이 생성(프록싱됨).
- **백엔드**: 터널 `localhostdaegu`(`14c7dc2e…`) → `api.localhostdaegu.cloud` → `127.0.0.1:8300`, `/docs·/redoc·/openapi.json`은 외부 404. systemd user 유닛 2개(`localhostdaegu-backend`·`localhostdaegu-tunnel`, linger on). 단일 워커, `CORS_ALLOW_ORIGINS=https://localhostdaegu.cloud,https://cloud-localhostdaegu.vercel.app`. 01:19 기동 수동 프로세스는 오늘 코드 미반영이라 교체.
- **프론트**: Vercel 프로젝트 `cloud-localhostdaegu`, CLI 배포(`frontend/`에서 `vercel deploy --prod` — Git 연동은 아직 안 함). 환경변수 2개(Production·Preview). `NEXT_PUBLIC_VWORLD_KEY`는 CLI가 자격증명으로 보고 막아 `--type config`로 명시. 인증서는 자동 발급이 안 돼 `vercel certs issue`로 발급(Let's Encrypt, ~12/18).
- **공개 주소 실측**: `/matching/myself` wired · 상담 매칭 **9건**(direct 2 + linked 7 — 런북의 "3건"은 OPEN-012 승격 전 값) · `/metrics/risk?industry=academy` 141개 행정동 · CORS 사전 요청 두 오리진 허용, 타 오리진 400 · `POST /finance/simulate` 외부 조달 필요 49,299,998 · **SSE가 Cloudflare를 지나도 버퍼링 없이 순차 도착**(0.2초 첫 이벤트, 1.4초 첫 `report_delta`, 12초대까지 분산) · 번들에 `api.localhostdaegu.cloud` 포함 확인 · `localhostdaegu.cloud` 4개 경로 200.
- **미결**: ① 브라우저 E2E는 다른 프로젝트의 Playwright가 돌고 있어 띄우지 않았다 — 화면 체크리스트(런북 §4)는 눈으로 확인 필요. ② 이 머신이 꺼지면 백엔드도 꺼진다(심사 기간 전원 유지). ③ Vercel Git 연동 없음 — 프론트 수정 시 CLI로 재배포. ④ `docs/deploy-runbook.md` §2의 head·상담 메타 분포가 옛 값.

### SGIS 지오코딩으로 학원·부동산 지표 연결, 어린이집 좌표 보정, 제안서 갱신

- **SGIS 지오코딩**(통계청·국가데이터처 `sgisapi.mods.go.kr`): 브이월드 지오코더가 결과 저장을 약관으로 금지해 SGIS로 대체했다. `store.address` 컬럼 추가(마이그레이션 `e3f4a5b6c7d8`) → 학원·부동산 재수집으로 도로명주소 채움 → `geocode_stores` CLI. 대상 12,631행이 **고유 주소 6,034개**로 접혀(주소 중복률 52%) 호출 6,037회, **1분 44초, 성공 6,034 / 실패 0 / 대구 범위 밖 0**. 권고 한도 일 5만 회의 12.1%. 캐시 `data/cache/sgis_geocode.csv`가 있으면 재실행은 API 0회(8,002행 복원 0.6초).
- **좌표 변환 실측 정정**: EPSG:5179 → WGS84는 브리프 예시값(128.4965, 35.8024)이 아니라 **(128.53752, 35.83849)** — 약 5 km 차이였다. 어린이집 원천 좌표 5건과 2~104 m로 교차검증. 무결과는 예외가 아니라 `errCd -100/-200`으로 온다(→ None, 실패도 캐시).
- **지표 연결**: `assign_regions` 12,631건 전량 기입(미판정 11건은 기존 cafe·restaurant 분) → `build_metrics` **7,856행 → 10,112행**. 학원·부동산 각 1,128행·행정동 141곳. 실서버 `/metrics/risk?industry=academy&year=2025`가 141개 행정동 랭킹을 돌려준다(작업 전에는 빈 배열).
- **폐업률은 관측 불가(원천 한계)**: NEIS·브이월드 NED는 현행 스냅샷만 주고 폐업분을 주지 않는다 → `close_count` 전 연도 0, 폐업 성분이 전 지역 동률이라 20.0(만점 40의 중간값)으로 고정되고 밀집·성장 성분만 움직인다(mid-rank 0.5 × 가중치 0.4). 어린이집 원천도 폐지 시설을 주지 않는다(휴지·재개 상태만). 소실 기반 추정은 후속.
- **어린이집 좌표 보정 15건**: 원천이 지오코딩 실패 시 **서울시청 좌표(37.56647, 126.977963)를 폴백으로 채운다** — 9건이 정확히 같은 좌표였다. 나머지 6건은 구 경계 밖. 15건 전부 재지오코딩·공간조인해 region 986/986. 재수집이 오류 좌표를 되돌려도 `source_coordinate_is_wrong()`(좌표만 보는 순수 판정 — 캐시 유무 무관)이 15건만 다시 잡는다.
- **제안서 갱신** `docs/plan/`: 과거 서술 정정 — "편의점·부동산·학원·어린이집 수집 전/등록만"을 실상태로(학원·부동산 **지표 연결**, 편의점 2,019·어린이집 986은 **수집 완료·지표 미연결**). 인허가 7업종 실측 161,141 → **161,163**, store 총계 **173,794**. `data-inventory` 차트 재생성(15개 묶음 **265,634건**, 스크립트 사본 `docs/plan/make_charts.py`), `licenses.md`에 NEIS·NED·소진공·어린이집포털·SGIS 5행 추가, 두 drawio 문구 갱신.
- **제안서 수치 재대조(09:50)**: 개발 DB 실측과 `docs/plan` 전수 대조. 추가 적재분(학원·부동산·편의점·어린이집·공공데이터 5종·지표 10,112)은 전부 일치. 매일 크론으로 늘어난 RAG·뉴스·공고만 어긋나 실측으로 갱신 — rag_chunk **3,589 → 3,643**(공고 1,735 = 기업마당 1,672 + 온통청년 63, 뉴스 1,908 · 334개 매체), news_article 1,913 → **1,914**, 15개 묶음 합계 **265,635건**. `plan.md` 6곳·과제요약(491자)·`licenses.md`·`tech-flow.drawio`·`make_charts.py` 수정, 차트 2장 재생성. `docs/model-evaluation.md`의 코퍼스 3,448건은 평가 당시 값이라 그대로 둔다. 좌표 없는 store 5,547건은 전부 기존 인허가 7업종(원천 누락)으로 이번 지오코딩 대상 밖.
- **RAG 평가셋 검수 시트**: 80건 전부 candidate(LLM 합성, 미검수)라 본지표가 없다 → `build_evalset_review` CLI로 `data/eval/review/rag_evalset_review.md`(질문·정답 청크 전문·만료·현재 검색 순위, 오답 시 1위 청크 첫 줄)와 판정 기입용 `.csv` 생성, `apply_evalset_review` CLI(confirm/revise/reject → jsonl 반영, .bak 백업, 순수 함수 테스트 4건)로 승격 경로 마련. 실측: funding 60건 중 만료 38, gemini-api@2560 기준 1위 적중 funding 47·news 9. 평가셋 1번(서산시 창업보육센터)처럼 온통청년 공고는 대구 밖 지역이 섞여 있어 검수 시 지역 적합성도 볼 것. 검수 자체는 사용자 몫(미진행).
- **라이선스 실측**: NEIS 학원교습소정보 "이용 허락 범위 제한없음"(갱신 매주) · 국토부 중개업정보 15123990 "이용허락범위 제한 없음" · 소진공 상가정보 15012005 "제한 없음"(개발계정 일 10,000회) · 어린이집 보육정보공개 API는 담당자 승인제(공공누리 유형 표기 없음).
- **미결**: ① SGIS 개발지원센터 약관 페이지가 열리지 않아 **지오코딩 결과 영구 저장 조건 미확인**(공공데이터포털 등록 건 15021230은 공공누리 제1유형). ② 편의점·어린이집의 `region_industry_metric` 연결. ③ 학원·부동산은 크론에 없다 — 수동 재수집 뒤 `geocode_stores`를 잊으면 좌표가 NULL로 돌아가 지표에서 두 업종이 사라진다. ④ `Store` 엔티티에 필드를 더하면 `StoreDto`·`StoreResponse`도 같이 고쳐야 한다(이번에 `address`로 실제 회귀 2건 발생, 수정 완료).

### 편의점·부동산 첫 수집, 어린이집 BC 이식·적재, 학원 대구 원천 확정

- **"편의점·부동산·학원·어린이집이 DB에 없는 건 키가 없어서인가"** — 아니었다. 편의점(소진공 sdsc2)·부동산중개업(브이월드 NED)은 키도 수집기도 있었고 한 번도 안 돌린 상태. 학원은 원천이 서울 전용이라 가드로 종료. 키가 없던 건 어린이집뿐.
- **편의점** `convenience_collector` 첫 실행: 144회 호출, **2,019건**(142개 행정동). 논공읍공단출장소·다사읍서재출장소 2건 실패 — 출장소는 소진공 API에 없는 행정동 코드라 응답 자체가 없고 본읍에 포함되므로 손실 없음. **부동산** `broker_collector`: 9회 호출, **4,629건**(8개 구·군, 좌표 없음 — 원천 설계대로 lat/lng NULL).
- **API 키 원본을 `backend/.env`로 이관**(사용자 작업). 루트 `.env`에는 docker-compose 치환용 `POSTGRES_*`만. 로더 우선순위 주석(`grid_keymaker_secret_manager.py`)과 `backend/.env` 안내문 정정.
- **어린이집 BC 이식**(Metabole `apps/childcare` 수집 경로만 — 조회 API·라우터는 가져오지 않음): 엔티티 2·포트·인터랙터·게이트웨이(`translate_http_errors` 적용)·리포지토리·ORM 2·수집기·마이그레이션 `d2e3f4a5b6c7`·테스트 10건. `CHILDCARE_API_KEY` 설정 추가. 실호출: 구·군 단위 arcode 1회에 구 전체(페이징 없음), **8회 호출 986건**(정상 926·재개 38·휴지 26 중 군위군 4건 제외), 좌표 986/986. 공간조인 **971건 기입 / 15건 미판정** — 9건은 좌표가 서울시청(37.566, 126.978) 자리표시자, 4건은 경산·영천 쪽(경도 128.79~128.94), 2건은 등록 구 밖 행정동(대신동·지산1동) 판정 → 좌표 오류 가드가 의도대로 배제. 백엔드 **507 passed / 1 skipped**.
- **군위군 스킵(사용자 결정)**: API는 27720도 4건 응답하지만 `district` 마스터에 없어 인허가·편의점·부동산과 같은 범위로 통일.
- **학원 대구 원천 확정**: 서울 OA-20528(`neisAcademyInfo`)의 원본인 **NEIS 교육정보 개방포털 `acaInsTiInfo`**. `ATPT_OFCDC_SC_CODE=D10`(대구교육청) 실호출 8,016건, `ADMST_ZONE_NM` 구 필터 동작(중구 441·달성군 762), 군위군 포함. 키 없이는 1회 5건(1,604회 필요), 키(즉시 발급·무료) 있으면 1회 1,000건 약 9회. 필드는 서울판과 1:1(PEI_NM→ACA_NM, ADMDST_NM→ADMST_ZONE_NM, REG_STTS_NM→REG_STTUS_NM, FLD_NM→REALM_SC_NM, TRNG_CRS_*→LE_CRSE_*, INDV_ATNLC_AMT_CN→PSNBY_THCC_CNTNT, ROAD_NM_ADDR→FA_RDNMA, LOAD_DT→LOAD_DTM). 좌표 없음 → 지오코딩 후속.
- **학원 수집 완료**: 사용자가 NEIS 키 발급(즉시) → `NeisAcademyGateway` 작성(서울 게이트웨이의 `_parse_ymd`·계열 매핑·상태 코드 재사용, `store_id=academy:neis:{ACA_ASNUM}`), `academy_collector` 서울 가드 제거·교체, `NEIS_API_KEY` 설정 추가. **파서 결함 발견**: NEIS 과정명은 "수학(초5, 초6):240000"처럼 괄호 안에 쉼표가 있어 서울판 쉼표 분리로는 금액이 유실 → 괄호 깊이 기준 분리기(`_split_top_level`)로 교체(+테스트). 실행 9회 호출 → **점포 8,002건·교습과정 21,646건**(수강료 있음 16,650), 군위군 14건 스킵. 구 분포 수성구 2,504·달서구 1,731·북구 1,171. 계열 매핑: 입시·보습 4,859·예체능 2,013·외국어 423·직업 224·독서실 108·미매핑 375(종합·기타·인문사회). 백엔드 **512 passed / 1 skipped**.
- **임베딩 불필요 판단**: RAG 원천은 funding 1,735·news 1,908 청크뿐이고 store·childcare는 결정론 지표 엔진 입력(구조화 데이터)이라 색인 대상이 아니다.
- **미결(후속)**: 학원 8,002·부동산 4,629건은 원천에 좌표가 없어 `region_code` NULL → `build_metrics`(region_code 보유분만 읽음)에 안 잡힌다. 도로명주소 지오코딩(브이월드 지오코더는 저장 금지 약관 — SGIS 지오코딩 검토) 후 `assign_regions`·`build_metrics` 재실행 필요. 어린이집 좌표 오류 15건도 같은 경로로 보정.

### 제안서 부록 — 화면별 기능 정리 4장, 사용자 흐름도, 경쟁 조사·STP·SWOT, ESG 정량 시나리오

- **화면별 기능 정리** `docs/plan/screen-1~4.png`: 실행 중인 프론트(3300)·백엔드(8300)를 headless Playwright로 시연 대본 값(중구 카페 4천만, 월세 250→100)대로 조작해 홈·지도·사전상담(비교표)·상담자료(리포트 완성)를 캡처하고, 화면 양옆에 기능·기술 설명 6개와 아래 5단계 흐름 띠를 붙여 합성. **첫 캡처에서 상담자료 진행 상황이 전부 "오류"** — 8300 백엔드가 9/18 20:03 기동 프로세스라 오늘의 2560 어댑터·halfvec ORM이 반영되지 않아(재색인된 DB와 불일치) 세 에이전트가 실패하고 있었다. 같은 인자로 재기동 후 전 에이전트 done 확인, 재캡처.
- **사용자 중심 시스템 흐름도** `docs/plan/user-flow.drawio`: 사용자가 하는 일 / 시스템이 하는 일 / 사용자가 얻는 것 3레인 × 6단계, 분기 3개(재무 입력 없이 분석 차단·조건 재변경·iM뱅크 후보 유무).
- **§3 차별성 브레인스토밍**: 유사 서비스 7개 조사(소상공인365·오픈업·나이스비즈맵·iM뱅크 앱 정책자금 사전진단·SOL Biz·지역신보 플랫폼·서울시 상권분석) — 보도자료·기사 확인 범위. 정량 비교 12축(데이터 규모는 비교 불가, 자금 4분리·비교표·상담자료·근거 등급은 우리만, 사용자 0은 약점), 정성 비교 5, STP(계약 검토 단계가 비어 있음, iM뱅크 앱 사전진단의 앞단), SWOT + 전략 4.
- **§4 ESG 브레인스토밍**: 외부 근거(서금원 컨설팅 이용자 폐업률 9.4% vs 11.7%·1년 생존율 +19.0%p, 폐업 시 68.5% 부채 평균 8,531만, iM금융 SOHO 컨설팅 3,550 사업자·임팩트 측정 방법론) + 가정을 명시한 정량 시나리오(연 485명 이용 가정 → 폐업 회피 5~6건·회피 부채 약 4.4억·상담 준비 시간 485시간) + 측정 설계. 성과가 아니라 시나리오임을 문두에 적음.
- 출처 목록 5-8을 16건으로 확장.

### 제안서 부록 — 임베딩 현황·데이터 건수 차트, draw.io 기술 플로우, 라이선스 정리

- `docs/plan/embedding-status.png`: 원천·기관별 청크 수(공고 1,693 = 기업마당 1,630 + 온통청년 63, 뉴스 1,896 = 333개 매체) 가로 막대, 총계·모델·차원·저장 방식 주석. `data-inventory.png`: 11개 묶음 225,029건 로그 눈금. matplotlib(스크래치 venv)로 그렸고 팔레트는 dataviz 검증(blue/orange ΔE 33.6 통과).
- `docs/plan/tech-flow.drawio`: 사용자 흐름 5단계 → API → FastAPI 7모듈 → AI 온·오프라인 → PostgreSQL+pgvector → 수집 크론, 기술 선택 표 12행. XML 파싱 검증만 했고 draw.io 렌더 확인은 안 함.
- `docs/plan/licenses.md`: 설치본에서 확인한 의존성 라이선스(런타임은 psycopg LGPL 외 전부 MIT·BSD·Apache), Pretendard OFL(단, CDN 참조라 오프라인 시연 시 self-host 필요), 공공데이터포털 6건 "이용허락범위 제한 없음·무료" 확인, Gemini 유료 API 데이터 미학습, 로컬 모델 Apache-2.0/MIT, EXAONE NC는 평가 기준점만. 미확인 항목(브이월드 약관 원문·15060371 이용허락·로고 출처) 체크리스트.
