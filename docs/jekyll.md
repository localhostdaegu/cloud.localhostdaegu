...
- **학원 스냅샷 규칙 통일**(`548586e`에 포함): 학원 인터랙터가 재수집 때 원천에 없는 좌표를 NULL로 덮어써 낮에 지오코딩한 8,002건의 lat/lng·행정동이 되돌아가는 결함이 있었다. 부동산(broker)과 같은 규칙으로 맞췄다 — 기존 행의 좌표·행정동 이월(`snapshot_support.carry_location` 공유), 스냅샷에서 사라진 학원은 폐업(추정) 처리. 학원 수집기 재실행으로 좌표·행정동이 보존되는 것을 확인했고 `test_academy_ingest` 단언을 새 규칙에 맞춰 갱신했다.
- **문서**: `docs/erd.md` §4-1에 비어 있는 테이블 3건(`shock_event_region`·`finance_product_category`·`consultation_document`)이 왜 비었는지 근거를 적고 테이블 수를 31로 정정. `docs/handoff.md`는 학원·부동산 스냅샷 운영 절차(좌표 이월·폐업 추정·크론 지오코딩)를 갱신하고, §0-1의 6번(온통청년 키)·7번(공공데이터·빈 테이블)을 완료 표시한 뒤 **§0-1-1 "남은 일 10건" 표**를 추가했다 — ① 배포 갱신+시연 영상(백엔드 환경변수 `NEIS_API_KEY`·`CHILDCARE_API_KEY`·`SGIS_*` 추가 필요) ② 제출 서류·접수(9/20) ③ 폐업 추정 라벨 유지(스냅샷 2~4주) ④ 좌표 없는 인허가 잔여 2,315건 ⑤ RAG 평가셋 80건 검수 ⑥ 재단 상품 취급은행 확인 ⑦ 센터 민간 데이터(본선 시 9/28) ⑧ 라이선스 잔여 확인 ⑨ 코드 소소한 이월 4건 ⑩ 제출 직전 크론 증감 수치 재갱신. SDD 계획서 2건(`docs/superpowers/plans/2026-09-19-sgis-geocode-and-plan-update.md`·`2026-09-19-snapshot-industry-metrics.md`)도 커밋.
- **커밋(23:08~23:11)**: `548586e` feat(store) 인허가 주소 저장·SGIS 지번 재시도·학원 스냅샷 규칙·지오코더 약관 준수 → `eea22fb` feat(metric) 편의점·어린이집 지표 연결 → `71d9d25` docs 작업 기록·ERD·handoff·SDD 계획 → `a0d474d` docs(handoff) 남은 일 10건. 낮 커밋 5건(`56e03a4`·`c6c09db`·`03012fa`·`00df250`·`21c0508`)까지 오늘 9건. 위 절들의 "미커밋" 표기는 이 시점에 해소됐다.
- **푸시 상태**: `origin/main`은 `00df250`(페르소나 후속 18건)까지 — Vercel Git 연동이 푸시마다 자동 배포한다(`03012fa` 푸시 때 75초 반영 실측, `00df250` 배포 결과는 따로 확인하지 않음). `21c0508` 이후 **5건은 미푸시**라 운영 프론트에는 AI 분석 예시 질문 칩·"편의점 (담배소매인 기준)" 라벨·정원·현원 카드 설명이 없고, 운영 백엔드는 재시작으로 작업 트리 코드(지표 11업종·정원·현원 카드)를 이미 서빙한다. 푸시는 사용자 결정(배포 갱신은 남은 일 ①).
- **제안서 반영**(`docs/plan`, git 미추적): 요약·§1-4·§2-4 표·§5-6·§5-7의 낡은 곳 7건 정정 — 테스트 수 526/203 → 603/226(46 files), "상권 지표 9종" → 11종 전부, 배포 "예정·실행 전" → 9/19 첫 배포·페르소나 10/10(밤 커밋분은 프론트 재배포 전), 소실 추정 규칙을 어린이집만이 아니라 학원·부동산·어린이집 셋으로 + "폐업률(추정)" 라벨, 로드맵 "본선 전 배포" → 프론트 재배포·시연 영상, 지도 진단 카드명 "성장률" → "점포 증감률". `licenses.md` 브이월드 행에서 지오코더·Address API 제거(SGIS 행으로 안내).
- **미결**: 위 10건 표 그대로. 오늘 밤 코드 변경은 더 없고, 백엔드 최종 실행은 **603 passed / 1 skipped**, 프론트 **226 passed**·`tsc` 0.

### 편의점·어린이집 상권 지표 연결 — 지표 원천 Strategy, 어린이집 정원·현원 카드

- **리뷰 후속(정직성 표기 보강)**: 리뷰 Important 1건 — 어린이집 카드가 "폐업률 0.0%"를 사실처럼 냈다. 원천이 폐업분을 주지 않는 세 업종(학원·부동산·어린이집)의 폐업률 카드 라벨을 "폐업률(추정)"로 오버라이드(dict)하고, 지표 해석 프롬프트에 "스냅샷 소실 추정, 실제 폐업으로 읽지 말 것" 주석을 같은 세 업종에 추가. Minor: `build()`가 두 원천의 (행정동·업종·연도) 중복을 조용히 덮어쓰던 것을 ValueError로 차단, `complete_year` 독스트링 갱신, AI 분석 예시 질문의 어린이집 첫 문구를 "정원 대비 현원"으로 교체(카드가 컨텍스트에 들어갔으므로). 백엔드 **603 passed / 1 skipped**, 프론트 226 passed. 데이터 건수 차트에 담배소매인 33,804 행 추가·지표 12,360(11업종)으로 재생성(16개 묶음 301,721건).
- **원천 Strategy**: `StoreStatsGateway`가 `store` 테이블 쿼리를 직접 들고 있던 구조를 `YearlyStatsSource`(ABC) + 레지스트리 합성으로 바꿨다. `StoreTableSource`(기존 인허가, 업종은 행마다) · `TobaccoProxySource`(담배소매인 → `convenience_store` 고정) · `ChildcareSource`(어린이집 → `childcare` 고정) 3개. 포트(`StoreStatsPort`)·`build_metrics`는 무변경이고, 새 원천은 `if`가 아니라 `_SOURCES` 리스트에 클래스를 더해 붙인다. 연말 영업 중·당해 개업·당해 폐업 계산과 **30일 규칙(OPEN-008)은 세 원천에 같은 함수로** 적용했다. 개폐업일 매핑은 담배 `coalesce(지정일, 인허가일)`/`coalesce(폐업일, 취소일)`(영업상태명은 갱신이 늦어 쓰지 않는다), 어린이집 `approved_on`/`coalesce(abolished_on, 소실 추정일)`.
- **어린이집 소실 추정**: 원천이 폐지 시설을 아예 반환하지 않아 `abolished_on`이 0건이다. 그 **구·군의 최신 관측일**(`max(last_seen_on)` by `district_code`)에 다시 보이지 않은 시설의 마지막 관측일을 폐원일로 본다. 전역 최대가 아니라 구·군 최대라 한 구의 수집 실패가 그 구 시설을 통째로 폐원시키지 않는다(Metabole 전례). 관측일이 하루뿐인 오늘은 소실 0건 — 규칙만 들어갔고 값은 스냅샷이 쌓인 뒤에 생긴다.
- **실측**: `region_industry_metric` 10,112 → **12,360행**, 지표 업종 **9종 → 11종**(등록 업종 전부). 편의점 1,136행·142개 행정동, 어린이집 1,112행·139개 행정동. 2025년 편의점(담배소매인 대용) 연말 영업 중 **6,208 · 개업 418 · 폐업 558**, 어린이집 시설 **962 · 인가 28 · 폐원 0**(소실 추정 규칙만 적용, 스냅샷 1일치).
- **정원·현원 카드**: master BC에 `ChildcareCapacityPort`·`ChildcareCapacityGateway` 추가. 운영 중 = 그 구·군 최신 관측일에 관측된 시설, 현황은 시설별 최신 `base_date` 1건만 합산한다. 지도 패널 카드는 업종→추가카드 dict로 붙이고(childcare만 항목), 포트 없이 구성한 geojson 경로는 Null Object가 받는다. **다사읍(2771025600)**: 어린이집 수 60개 · 폐업률 0.0% · 점포 증감률 +0.0% · **정원 대비 현원 1769/2702 (65.5%)** · **입소대기 1418명**(기준일 2026-09-19). 첫 카드 60개는 2025년 말 기준, 정원 카드의 62곳은 현재 관측 시설이라 기준이 다르다. 카드는 `MarketDataGateway`가 그대로 넘기므로 AI 리포트 지표 해석에도 들어간다(analysis 무변경).
- **정직성 표기**: 지도 패널 첫 카드 라벨 `점포수(담배소매인 기준)`·`어린이집 수`(업종→라벨 dict, "데이터 없음" 경로도 같은 라벨), 프론트 업종명 `편의점 (담배소매인 기준)`, 리포트 `market_prompt`에 "편의점 지표는 담배소매인 지정 현황 대용이며 슈퍼마켓·마트가 함께 포함된다"를 편의점일 때만 한 문장 덧붙였다(`industry.name`은 무변경). `industry_source_code`에 `(convenience_store, mois_permit_tobacco, 기타_담배소매업)` 시드 추가.
- **검증**: 백엔드 pytest **600 passed / 1 skipped**(+23), 프론트 vitest **226 passed**, `tsc` 0. 실서버(8300)가 새 코드로 `/metrics/risk?industry=convenience_store|childcare`와 다사읍 카드 5장을 응답하는 것까지 확인.
- **한계·미결**: ① **담배소매인 ≠ 편의점** — 슈퍼·마트 등 담배 판매점이 함께 잡히고, 담배를 안 파는 편의점은 빠진다. 6,208은 편의점 점포 수가 아니라 지정 건수다. ② 어린이집 폐업률은 전 지역 0이라 위험도의 폐업 성분이 동률이다(학원·부동산과 같은 상태) — 스냅샷 2~4주 후 재평가. ③ 입소대기 1,418명은 중복 신청을 포함한 원천 건수이고 공란 시설은 합산에서 빠진다(전부 공란이면 "미공개"). ④ 어린이집의 나머지 두 카드 라벨("폐업률"·"점포 증감률")은 그대로 뒀다. ⑤ ~~미커밋~~ → 23:08 `eea22fb`로 커밋(미푸시).

### 인허가 좌표 없음 5,547건 지오코딩, 온통청년 키 교체, 제안서 수치 일괄 갱신

- **지오코딩 약관 2건 확인·교체**: 브이월드 Geocoder API 2.0 레퍼런스 원문 "API 요청은 실시간으로 사용하셔야 하며 별도의 저장장치나 데이터베이스에 저장할 수 없습니다" — 낮에 공공데이터 지표용으로 캐시·적재한 역 91·백년가게 82건이 위반. SGIS 이용약관(`developer/html/openApi/app/rules.html`)은 저장·재배포 금지 조항 없음(일 5만 회, 영업활동은 사전 승낙, 공공누리 1유형 출처 표시). 조치: `normalize_address`를 `core/matrix/grid_address_normalizer.py`로 이동(순환 import 해소, 리뷰 이월 건), `load_open_data`의 지오코더를 SGIS 게이트웨이로 교체(브이월드는 전통시장 Data API만), 역 94·백년가게 83곳 SGIS 재지오코딩 전건 성공 → `regional_indicator` 재적재(역 2,632행 동일, 백년가게 46→47행), 브이월드 결과 CSV 저장소에서 제거. licenses.md 정정.
- **온통청년 키 교체**: 9/17부터 매일 05:10 실패하던 funding 크론의 원인이 `invalid api key`(403)였다. 사용자가 마이페이지에서 청년정책 API 키를 재발급 → 단건 200(전국 2,819건)·수집기 exit 0(온통청년 63건, 신규 0 — 대구 구·군 코드 붙은 창업 정책이 9/17 이후 신규 없음).
- **인허가 좌표 없음 5,558건**: 원천에 관리번호 조회가 없어 행안부 게이트웨이가 `ROAD_NM_ADDR`(없으면 `LOTNO_ADDR`)를 `store.address`에 담도록 고치고 7업종 `--full` 재수집(161,185건, 실패 0). SGIS 1차: 5,547건 중 2,944 성공·2,594 실패(47%). 실패의 91%가 "송현동 554-2"류 지번 — SGIS 실측으로 부번 지번은 무결과·본번("554")은 결과, 앞자리 0("0067-0003")·층호도 방해 확인 → 게이트웨이에 **주소 변형 재시도**(원문 → 0 제거·뒤 토큰 제거 → 부번 제거, 첫 성공에서 멈춤; 법정동 중심점 단계는 행정동 편향 때문에 두지 않음)와 CLI `--retry-failed`(캐시 실패분 재질의) 추가(+5 테스트). 2차: +288 성공. **잔여 2,315건**(영업 중 35, 2019~2025 폐업 37 = 전체 폐업 37,296의 0.1%)은 SGIS에 없는 옛 지번이라 지표 영향 미미 — 도로명주소 API(juso.go.kr) 변환은 후속.
- 공간조인 158,570 → 인허가 좌표 보유 158,870(98.6%)·행정동 158,859. `build_metrics` 재실행(10,112행, 값 변동). **크론 `store-collector.sh`에 `geocode_stores`(캐시 우선) 단계 추가** — 증분 merge가 좌표 없는 행의 lat/lng를 비우기 때문. 백엔드 **577 passed / 1 skipped**.
- **제안서 수치 일괄 갱신**(사용자 요청, DB 스냅샷 22:30): 점포 173,816(인허가 161,185)·뉴스 1,926(337개 매체, 본문 5곳 1,914/1,908 혼재 → 통일)·공고 1,735·색인 3,643·합계 265,669. 재집계 후 7업종 2025년 개업 4,026·폐업 5,331, 2024년 4,778/5,284, **2023년은 개업 5,377 > 폐업 5,351**로 "3년 연속 폐업 > 개업" 서술을 "2024~2025 2년 연속"으로 정정. 일반음식점 2025년 말 영업 중 29,380. data-inventory 차트 재생성(그림 1 미변경), licenses.md SGIS 행을 고유 주소 10,853·성공 8,884로.

### AI 분석 추가 질문 — 업종별 예시 문구와 클릭 칩

- 사전상담·AI 분석 폼의 추가 질문 placeholder가 "원두 가격이 오르면 손익분기점이…"로 하드코딩돼 있었다. 카페 전용인 데다 손익분기 계산은 결정론 엔진 몫이라 LLM 섹션이 답할 수 없는 질문이었다. 추가 질문은 판정·지표 해석·대구 뉴스와 충격·자금과 상품 4개 LLM 섹션 프롬프트에 붙는다(`report_text.py`).
- `features/agent-report/lib/example-questions.ts`: 11개 업종 × 3문구 사전. 각 업종에 지표 → 뉴스·충격 → 자금·선택 축을 하나씩 배분하고 계산 질문은 제외(테스트로 "손익분기|매출" 금지). 학원·부동산은 폐업분이 없어 밀집·증감·인구·금리로, 편의점·어린이집은 지표가 없어 뉴스·자금·입지만. 미등록 업종은 일반 문구 1개 폴백.
- `analysis-form.tsx`: placeholder = 업종 첫 문구, 입력칸 아래 칩 3개(누르면 입력칸을 채움, 업종을 바꿔도 적은 질문은 유지). 백엔드 무변경. vitest 226 passed(+7), tsc 0, headless 캡처로 카페→학원 전환·칩 클릭 확인.
- 후속: 어린이집 첫 문구는 정원·현원(`childcare_center_stat`)이 리포트 컨텍스트에 붙으면 "정원 대비 현원" 질문으로 교체.

### 페르소나 테스트 후속 개선 18건 — 상담자료 문구·의도 해석·모바일 지도·조정 안내

- **상담자료(백엔드 `apps/analysis`)**: ① `plan_prompt`에 계획 수치를 그대로 싣고 "수치가 없다고 쓰지 마라"를 명시 — 첫 문단의 자기부정 해소(재실행 10건 중 0건). 변경 이유는 있을 때만 알린다. ② 금액을 화면과 같은 만원 단위로(`krw()` — 97,421,998원 → 9,742만원, 1억 이상은 "2억 1,085만원"). ③ 최초안=현재안이면 값이 같은 표 대신 "바꾼 조건이 없습니다". ④ 지표가 전부 없는 업종은 LLM을 부르지 않고 "집계되지 않았을 뿐 상권이 나쁘다는 뜻이 아니다"를 코드가 쓴다. ⑤ **상품 수 불일치 원인 확인·수정**: 화면은 상담 후보(`build_consultation_candidates`), 상담자료는 옛 `match_products`였다 → 상담 정보가 있으면 같은 함수·같은 입력(프로필·자치구)을 쓴다. ⑥ 참고 자료에서 다른 구·군만 가리키는 문서 제외(`district_relevance.keep_relevant`, '달서구' 안의 '서구' 오인 방지, 2배수 검색 후 자름). ⑦ 진행 패널 문구에서 "RAG"·"스코어" 제거.
- **의도 해석(`apps/intent`)**: 편의점·학원·부동산·어린이집 + 국수·국밥·분식 등 12개 단어, 경대북문·경대·경대병원(삼덕동, "경대"보다 길어 먼저 검사). 응답에 `region_code` 추가(추가만) → 랜딩이 `region`을 실어 보내고 지도가 **진입 시 1회** 그 동 경계로 화면을 맞춘다. 서문시장 → 대신동, 칠성시장 → 칠성동 선택 상태로 진입 확인.
- **랜딩**: 되물음 칩을 등록 11업종 전체로. 동네를 못 알아들으면 말없이 대구 전체로 보내지 않고 구·군 8개를 되묻는다("아직 몰라요 — 대구 전체 보기" 포함). 쓰지 않게 된 7업종 부분집합(`shared/daegu.ts`) 제거.
- **지도**: 768px 미만에서 지도(위 42dvh)·패널(아래)로 쌓는다 — 400px에서 지도가 80px만 남던 문제. 업종 미정이면 상단 select가 "업종 미정 — 지도 색은 카페 기준". "데이터 없음" 카드에는 "확인된 사실" 배지를 달지 않는다. "성장률" → **"점포 증감률"**(백엔드 카드 라벨·지표 토글·mock).
- **사전상담**: "빌리기 전에 줄여 볼 수 있는 것" — 영향이 큰 비용 3개를 10% 줄였을 때 준비자금 감소액(월 비용은 운영준비금 개월 수를 곱한다, 엔진 관계식 그대로의 산수) + "다른 점포·계약 보류도 계획". 모바일에서 주 지표 1열·줄바꿈 방지. 상품 사유 문구도 만원 단위.
- **상담자료 화면**: "이 안으로 상담 준비"로 오면 자동 시작(`autostart=1`, 시작과 동시에 URL에서 제거 — 새로고침 재생성 방지). 버튼은 "상담자료 만들기".
- **검증**: pytest **572 passed / 1 skipped**, vitest **219 passed**, `tsc` 0, 실백엔드 `funnel.cjs`·`analysis.cjs` RESULT: PASS(analysis.cjs 금액 정규식을 새 단위에 맞춤). **같은 10명 재실행(로컬 빌드 + 운영과 같은 백엔드): 10/10 도달, 마찰 기록 0건**, 자기부정 문단 0/10, 원 단위 표기 0/10. 결과·스크린샷 `data/cache/persona-test-2026-09-19/after/`.
- **하지 않은 것·미결**: ① 리뷰 경로(`purpose=review`)의 등급 문구 "진입 주의"는 그대로 — 모델 평가 하네스(`direction_ok`)가 이 단어에 걸려 있다. ② 모바일 패널의 하단 고정 버튼은 패널을 스크롤해야 보인다. ③ 자동 시작 뒤에도 버튼으로 다시 만들 수 있다(의도된 동작이지만 LLM 호출이 또 나간다). ④ ~~미커밋·프론트 미배포~~ → 12:51 `00df250`으로 커밋·푸시(Vercel 자동 배포 대상). (기록 당시) 백엔드는 재시작해 운영에 반영됨. 운영 프론트는 이전 빌드라 추가 필드·라벨 변경과 섞여 돌고 있다(추가만 한 계약이라 동작은 정상, "점포 증감률" 카드 설명만 빠진다).

### 운영 사이트 페르소나 고객 테스트 10명 — 전원 상담자료 도달, 결함 7건 발견

- **방법**: 푸시(`0f51701..03012fa`) → Vercel 자동 배포 75초 만에 반영 확인 → `https://localhostdaegu.cloud`에서 헤드리스 Playwright 1개로 10명을 순차 실행. 각자 자기 말투로 랜딩에 입력 → 지도 가운데를 눌러 동 선택 → 사전상담 입력 → 상담자료 생성(저장 버튼 활성)까지. 스크립트·결과 JSON·스크린샷 53장: `data/cache/persona-test-2026-09-19/`(git 미추적).
- **결과**: **10/10 목표 도달**, 1인 평균 **30.6초**(자동 입력 기준), 상담자료 생성 평균 **11.2초**, 콘솔 오류는 정상 404(위험도 데이터 없음) 외 0건. 월세 도우미(60평 두류감삼역 중대형 → 493만원), 월세 350 → 200 비교(손익분기 ▼194만원·준비자금 ▼900만원), 업종 미정 → 랭킹 9업종 → 선택, 모바일(400px) 모두 끝까지 동작.
- **결함 ① 상담자료 첫 문단이 스스로를 부정한다(전원 해당)**: 코드가 쓴 수치 바로 아래에 LLM이 "제공된 계획에는 구체적인 자금 수치나 세부 내역이 제시되어 있지 않아…"라고 쓴다. **원인 확인**: `plan_prompt`가 "위 수치는 이미 표에 있으니"라고만 하고 수치를 프롬프트에 넣지 않는다(`backend/apps/analysis/domain/report_text.py:221`). 변경 이유 미입력도 "'없음'으로 기재되어 모호"하다고 지적한다.
- **결함 ② 랜딩이 등록 업종 4종을 못 알아듣는다**: "편의점·학원·부동산·어린이집"이 동의어 사전에 없고 되물음 칩(7종)에도 없다 → 편의점 고객은 "잘 몰라요"로 들어가 랭킹에도 없어 상단 select에서 직접 골라야 했다. "국수집"·"분식집"도 미인식, 랜드마크 "경대북문" 미인식(C유형으로 떨어짐).
- **결함 ③ 말한 동네가 지도에 전달되지 않는다**: 파서는 서문시장→대신동, 칠성시장→칠성동, 동성로→성내1동을 알지만 URL에는 구·군만 실린다. 지도는 구 중심으로만 이동해 "서문시장 근처"라 말한 고객이 동인동을 고르게 됐다.
- **결함 ④ 모바일 지도 사용 불가 수준**: 400px에서 옆 패널이 320px 고정이라 지도가 80px만 남고 범례가 찌그러진다. 사전상담 주 지표는 3열에서 "8,061만/원"으로 줄바꿈.
- **결함 ⑤ 데이터 없는 업종의 표기**: 편의점 패널이 "점포수 데이터 없음" 옆에 **"확인된 사실"** 배지를 단다. 리포트 상권 진단도 "데이터가 전혀 제공되지 않아" 문장 3개로 채워진다.
- **결함 ⑥ 단위 불일치**: 화면은 만원(9,742만원), 상담자료는 원 단위 전체 자릿수(97,421,998원). 은행에 가져갈 문서로 읽기 어렵다. 한 번만 계산한 고객에게도 값이 같은 "최초안·현재안" 비교표가 나온다.
- **결함 ⑦ 참고 자료 지역 소음**: 달성군 고객의 참고 자료에 수성구·동구·북구 자금 공고가 섞인다.
- **한계**: 자동화된 가상 고객이라 이해도·신뢰감은 재지 못했다. 동 선택은 "지도 가운데 클릭"으로 고정했고, 입력값은 미리 정한 수치다. 사람 대상 검증은 아니다.
...
- **제안서 집계 수치 재집계 반영**(오후, 03012fa의 30일 이내 종료 제외 재빌드 뒤): `docs/plan/plan.md`의 7업종 2025년 개업 4,854·폐업 6,259는 제외 전 산식(region 보유분) 값임을 DB로 재현 확인 → 재집계 값 **개업 3,879·폐업 5,301**(2023년 5,291/5,328·2024년 4,638/5,256, 3년 연속 폐업 > 개업 유지)로 3곳 교체. 일반음식점 2025년 점포 수 29,113 → `store_count` 29,105. 30일 이내 비율(카페 27.2%·음식점 12.6%)과 행정동 커버리지(137~142)는 불변. 연간 대상자 상한 행에 제외 전 원천 기준 5,004건 병기.
