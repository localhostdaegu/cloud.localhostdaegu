...
it("등록된 region·industry는 200과 점포 배열을 반환한다", async () => {
  const res = await GET(new Request("http://test/api/mock/stores?region=2711051000&industry=cafe"));
...
it("지원하지 않는 industry는 404 INDUSTRY_NOT_FOUND를 반환한다", async () => {
  const res = await GET(new Request("http://test/api/mock/stores?region=2711051000&industry=unknown_industry"));
  expect(res.status).toBe(404);
  expect((await res.json()).error.code).toBe("INDUSTRY_NOT_FOUND");
});

it("실적재 데이터가 없는 업종은 (region·industry 모두 유효해도) 빈 배열을 반환한다", async () => {
  const res = await GET(new Request("http://test/api/mock/stores?region=2711051000&industry=convenience_store"));
  expect(res.status).toBe(200);
